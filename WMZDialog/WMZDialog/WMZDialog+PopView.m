...
- (UIView*)popViewAction{
    ...
    self.tableView.frame = CGRectMake(0, 0, self.wWidth, self.wMainBtnHeight*([self.wData count]>10?10:[self.wData count]));
    ...
    if (direction == directionUp) {
        rect.origin.x = tapRect.origin.x*self.wPercentOrginX;
        //超出屏幕方向变为下面
        if (CGRectGetMinY(tapRect) -self.wMainOffsetY - rect.size.height <0) {
